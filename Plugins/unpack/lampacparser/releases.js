...
const KINOZAL_URLS = {
  kinozal_4k: {
    ...
    tmdbType: 'movie',
    category: 'movies',
    pages: 12 // Количество страниц для фильмов в 4K
  },
  kinozal_1080p: {
    url: 'https://kinozal.tv/browse.php?c=1002&v=3001&page=',
    tmdbType: 'movie',
    category: 'movies',
    pages: 12 // Количество страниц для фильмов в 1080p
  },
  kinozal_tv_4k: {
    url: 'https://kinozal.tv/browse.php?c=1001&v=7&page=',
    tmdbType: 'tv',
    category: 'tv',
    pages: 0 // Количество страниц для сериалов в 4K
  },
  kinozal_tv_1080p: {
    url: 'https://kinozal.tv/browse.php?c=1001&v=3001&page=',
    tmdbType: 'tv',
    category: 'tv',
    pages: 0 // Количество страниц для сериалов в 1080p
  }
};
...
function filterAndProcessItems(items) {
  ...
  items.forEach(item => {
    ...
    // Фильтрация по старости раздачи (добавляем в конец перед push)
...
function groupByCategory(normalizedData) {
  const categoryTitles = {
    'movies': 'Фильмы',
    'tv': 'Сериалы',
    'kinozal_4k': 'Кинозал 4K',
    'kinozal_1080p': 'Кинозал 1080p',
    'kinozal_tv_4k': 'Сериалы 4K',
    'kinozal_tv_1080p': 'Сериалы 1080p',
    'movies_foreign': 'Зарубежные фильмы',
    'movies_russian': 'Русские фильмы',
    'tv_foreign': 'Зарубежные сериалы',
    'tv_russian': 'Русские сериалы',
    'foreign': 'Зарубежное',
    'russian': 'Русское'
  };
  ...
  normalizedData.results.forEach(item => {
    item.categories?.forEach(catId => {
      ...
      // Вместо сокращенного объекта - кладем полные данные
      categoriesMap[catId].items.push(item);
    });
  });

  // Сортируем категории по количеству элементов
...
function normalizeCustomJson(items) {
  ...
  // Создаем временную карту для группировки по категориям
  ...
  for (const key in groups) {
    ...
    // Проверяем год из TMDB
    let itemYear;
    if (bestItem.media_type === 'movie' && bestItem.release_date) {
      itemYear = new Date(bestItem.release_date).getFullYear();
    } else if (bestItem.media_type === 'tv' && bestItem.first_air_date) {
      itemYear = new Date(bestItem.first_air_date).getFullYear();
    } else {
